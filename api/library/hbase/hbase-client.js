...
HbaseClient.prototype.getAccounts = function(options, callback) {
  ...
  // counts over time
    ...
    params = {
      table: 'agg_stats',
      startRow: options.interval + '|' + utils.formatTime(options.start),
      stopRow: options.interval + '|' + utils.formatTime(options.end),
      columns: ['metric:accounts_created']
    };
  ...
  // individual rows
    ...
    params = {
      table: 'accounts_created',
      startRow: utils.formatTime(options.start || 0),
      stopRow: utils.formatTime(options.end)
    };
